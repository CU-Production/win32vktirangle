...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg)
    {
        ...
        case WM_DPICHANGED:
            ...
            int dpiScaledX = MulDiv(suggested_rect->left, iDpi, USER_DEFAULT_SCREEN_DPI);
            int dpiScaledY = MulDiv(suggested_rect->top, iDpi, USER_DEFAULT_SCREEN_DPI);
            int dpiScaledWidth = MulDiv(suggested_rect->right - suggested_rect->left, iDpi, USER_DEFAULT_SCREEN_DPI);
            int dpiScaledHeight = MulDiv(suggested_rect->bottom - suggested_rect->top, iDpi, USER_DEFAULT_SCREEN_DPI);
...
void create_window_win32(const char* window_name) {
    ...
    // AdjustWindowRectExForDpi(&rect, style, FALSE, exStyle, GetDpiForWindow(init.hwnd));
    ...
    // init.hwnd = CreateWindowExW(exStyle, init.wc.lpszClassName, L"win32 native vulkan tirangle demo", style, frameX, frameY, frameWidth, frameHeight, NULL, NULL, init.wc.hInstance, NULL);
    init.hwnd = CreateWindowW(init.wc.lpszClassName, L"win32 native vulkan tirangle demo", WS_OVERLAPPEDWINDOW, frameX, frameY, frameWidth, frameHeight, NULL, NULL, init.wc.hInstance, NULL);

    ShowWindow(init.hwnd, SW_SHOWDEFAULT);
    UpdateWindow(init.hwnd);
}

void destroy_window_win32() {
    DestroyWindow(init.hwnd);
    UnregisterClassW(init.wc.lpszClassName, init.wc.hInstance);
}

int device_initialization() {
    create_window_win32("Vulkan Triangle");
    ...
    win32_surface_create_info.hwnd = init.hwnd;
    win32_surface_create_info.hinstance = init.wc.hInstance;

    if (vkCreateWin32SurfaceKHR(init.instance, &win32_surface_create_info, NULL, &init.surface) != VK_SUCCESS) {
