...
#endif
const int enableRenderdocCapture = 1;
...
int init_data() {
    ...
    init.swapchain.image_format = VK_FORMAT_R8G8B8A8_SRGB; // The image format actually used when creating the swapchain.
...
void create_window_nn(const char* window_name) {
    nn::vi::Initialize();

    nn::Result result = nn::vi::OpenDefaultDisplay(&init.g_pDisplay);
    NN_ASSERT(result.IsSuccess());
    NN_UNUSED(result);

    result = nn::vi::CreateLayer(&init.g_pLayer, init.g_pDisplay);
    NN_ASSERT(result.IsSuccess());

    nn::vi::SetLayerCrop(init.g_pLayer, 0, 0, SCR_WIDTH, SCR_HEIGHT);
}

void destroy_window_nn() {
    nn::vi::DestroyLayer(init.g_pLayer);
    nn::vi::CloseDisplay(init.g_pDisplay);
    nn::vi::Finalize();
}

int device_initialization() {
    create_window_nn("Vulkan Triangle");
    ...
    const char* glfwExtensions[2] = {
        {"VK_KHR_surface"},
        {"VK_NN_vi_surface"}, // VK_NN_VI_SURFACE_EXTENSION_NAME
    };

    const char** extensions;
    uint32_t extension_count;
    if (enableValidationLayers) {
        extension_count = glfwExtensionCount + 1;
    ...
    layer_names.clear();
    layer_names.push_back("VK_LAYER_NN_vi_swapchain");
    if (enableValidationLayers) {
        layer_names.push_back("VK_LAYER_KHRONOS_validation");
    }
    if (enableRenderdocCapture) {
        layer_names.push_back("VK_LAYER_RENDERDOC_Capture");
    }
    ...
    nn::vi::GetNativeWindow(&nativeWindow, init.g_pLayer);

    VkViSurfaceCreateInfoNN nn_vi_surface_create_info = {};
    nn_vi_surface_create_info.sType = VK_STRUCTURE_TYPE_VI_SURFACE_CREATE_INFO_NN;
    nn_vi_surface_create_info.window = nativeWindow;

    if (vkCreateViSurfaceNN(init.instance, &nn_vi_surface_create_info, NULL, &init.surface) != VK_SUCCESS) {
        printf("failed to create nn_vi surface!\n");
        return -1;
    }
    ...
    device_create_info.enabledLayerCount = layer_names.size();
...
int create_swapchain() {
    ...
    for (uint32_t i = 0; i < imageCount; i++) {
        VkImageViewCreateInfo image_view_cre_infos = {};
...
extern "C" int nnMain() {
    ...
    void* pGraphicsSystemMemory = aligned_alloc(4096, g_GraphicsSystemMemorySize);
    nv::InitializeGraphics(pGraphicsSystemMemory, g_GraphicsSystemMemorySize);

    /* Initialize input for user exit */
    nn::hid::InitializeTouchScreen();
    nn::hid::TouchScreenState<1> touchScreenState;
    int touchCount = 0;
    NN_UNUSED(touchCount);
